...
incidents_wvu <- incidents_wvu %>%
  ...
  mutate(incident_date=mdy(datetime)) %>% 
  mutate(incident_time=hour(datetime)) %>% 
  mutate(day_of_week=day(datetime)) %>% 
...
1. A dataframe with the crime log data loaded and the datetime column converted to the proper datatype.(working)
2. Columns in your dataframe that contain the date, time and day of the week that each date represents (you may need to google how to do that).(working)
3. Generate a dataframe of the most recent date's incidents.

```{r}
#calculating percentage change
#take the last date, and take the 7 days before that date.
latest_date <- incidents_wvu %>% slice(1)
#above is a one row dataframe
#below targets that one row dataframe with the $, and -7 is saying 7 days before it.
one_week_ago <- incidents_wvu %>% filter(incident_date == latest_date$incident_date - 7)
latest_date <- latest_date %>%
  mutate(pct_change_week = (title - one_week_ago$title)/one_week_ago$title *100)
```


```{r}
#original from prelab
