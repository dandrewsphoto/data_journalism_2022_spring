...
#tidycensus is aware of census data at many local levels - not just the data of the geography by the maps of the data.
#have to sign up with a key to use it with the API
#google census API key - https://api.census.gov/data/key_signup.html
library(tidycensus)
```
...
#load variables is saying i just want to see whats available from the census dataset.
#sf1 is one of the datasets that the census has, there are several of them. sf1 and sf3 pertain to the count every 10 years, the decennial census, the others deal with surveys they send out.
#acs1 - would give you state demographic data from the last year, acs5 would give you down below town level
#sf1 has almost 9,000 different variables.
#sf1 is a guide of what to ask for. An index. 
...
# options for geography value are: state, county, place, tract, blocks
#also here's the diagram - https://www2.census.gov/geo/pdfs/reference/geodiagram.pdf
```
...
**Answer** Nevada with a 35% change.


```{r}
alldata %>% mutate(change = ((Population2010-Population2000)/Population2000)*100) %>% arrange(desc(change))
#how to calculate percentage change - (1new minus old) divided by the old
#lots of population growth in the four corners area, lots of land, lower taxes, lower cost of living, warm states.
#lots of population loss in the northeast where its cold, and then there's also louisiana --> hurricane katrina.
```
...
#it took a long time to do a census during the pandemic, its taken even longer to publish the data. There's no sf file yet.
#thats why we use the acs, the survey. Because its more recent, but its an estimate. you have to factor in margins of error.
View(pl)
```
...
md <- get_acs(geography = "county",
              ...
              #creating the column called medincome
              state = "MD",
              year = 2019)
...
md
#has a column called ESTIMATE, not value. MOE is Margin of Error. That means Calvert and Montgomery are pretty close as possible number 2s, depending on the Margin of Error. 
```
...
**Answer** No, they're basically the same median income just with different margins of error.

Is the difference between Howard and everyone else significant?
**Answer** Yes. even with Howard with a sizeable MOE, it's still no where close to #2. 
...
**Answer** Somerset and Caroline County saw their median incomes fall. Somerset’s margin of error was 6664 in 2019, Caroline’s MOE was 3620. I'm not really sure if that's within their margin of error or not.

```{r}
md10 %>%
  inner_join(md19, by=c("GEOID", "NAME")) %>%
  #join md10 with md19 by using interjoin and combining the columns GEOID and NAME
  rename(estimate2010=estimate.x, estimate2019=estimate.y) %>%
  #rename the above column names
  mutate(min2010 = estimate2010-moe.x, max2010 = estimate2010+moe.x, min2019 = estimate2019-moe.y, max2019 = estimate2019+moe.y) %>%
  #create new columns based on what we renamed them above
  select(-variable.x, -variable.y, -moe.x, -moe.y) %>%
  #select
  mutate(change = ((estimate2019-estimate2010)/estimate2010)*100) %>%
  #create a column called change by calculating the percent cchange
  arrange(change)
#arrance in ascending order
```
...
**Answer** 21201, 21202 but MOE is 6931 which could put it above 50k, and 21740 but MOE is 2635 which could put it above 50k.

```{r}
md_zipcodes <- get_acs(geography="zcta", variables = "B19013_001", state='MD')
#start with renaming a dataframe md_zipcodes, get acs data, including zcta for geography, variables, and the state of MD
md_zips_with_acs <- top_zips %>% 
  left_join(md_zipcodes, by=c('zip5'='GEOID'))
#take top zips and rename it md_zips_with_acs, then use left_join to join it with md_zipcodes by zip5 and GEOID.
