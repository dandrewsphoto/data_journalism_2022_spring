...
**Answer** A bunch of information files of different kinds.
...
**Answer** location of hospitals, long and lat, how many beds in each, type of hospital.
...
**Answer** data on the type each hospital is.
...
md_hospitals
#72 hospitals in Maryland, where they're located, type of hospitals, etc. Mostly general acute care hospitals. 
```
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** a round up of how many of each types of hospitals are in maryland.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** maryland hospitals, filtered for just general acute care.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** a scatter plot that sort of looks like Maryland without the borders we know.

```{r}
#uses ggplot to plot them on the grid. geom_sf will give you a blank grid to plot things on.
#you can plot places on a grid with any well-defined space. as longas you have a grid coordinate system, you can put points on a grid.
md_hospitals %>%
  ggplot() + 
  geom_sf() + 
  theme_minimal()
#this map looks like Maryland!
```
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** Installed tigris for future use.

```{r}
# install.packages('tigris')
library(tigris)
#this will give you geographic context, another geographic layer on which to put your scatterplot of dots.
```
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** county shapefile, but with a geography column.
```{r}

counties <- counties() 
#a tigris command that gives you a dataframe of counties.
#geometries column of counties has a weird shape to let you know its creating a shape.
glimpse(counties)
```
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** filters just for maryland.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** the state of maryland with defined counties.
...
md_counties %>%
  ...
  geom_sf() + 
  theme_minimal()
#we will superimpose our datapoints on this map.
```
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** maryland witht he datapoints superimposed on top. 
...
ggplot() + 
  ...
  theme_minimal()
#inputting each layer seperately. The counties, then the hospitals. These are layers you're adding ontop of each other.
#can now see exactly where the hospitals are in the state, but its very binary. Not all hospitals are the same. 
```
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** improved map of the dots on Maryland, color coded by range.
...
ggplot() + 
  ...
  #adding aesthetics and color palletes to make them easier to distinguish.
  scale_colour_viridis_b(option="magma") + 
  theme_minimal() 
#seperate them by the number of beds, the type, etc.
#we can keep adding in data to this.
#if we have counties, we can get cenus data for counties.
...
#dont need to uncomment that line if you're on the same laptop and have already loaded a census api key.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** the types of hospitals in maryland, filtered just for general acute care.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** number of beds in each maryland county, arranged in descending order.

```{r}
#calculate the number of beds by county.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** populationf or each county in maryland.

```{r}
#get county population from MD
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** census population data with geometry column so that the map knows where to put it.

```{r}
#how figure out a per capita rate of beds per population.
md_county_population <- get_acs(geography = "county", 
              variables = c(population = "B01001_001"), 
              state = "MD",
              geometry = TRUE)
#geometry = True: "oh by the way, make this census map aware. Make so we can use it in a MAP."
#this gives you a dataframe that has not just the population, but also the geometry column. You can then use it on a map.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** county population, and number of beds per county.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** a cleaned up lst of county names.

```{r}
#join the beds per county and the per capita per county. But first you need to clean it up so they can be joined.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** a joined dataframe of number of beds per 100k and md county populations.
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** the numner of total beds is replaced to zero if its currently na.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** a new columun is created to calculate the total number of beds per the county population x 100k.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** a colored map of maryland where baltimore and alleganey county have the most beds per capita.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** a better version of a maryland map, distinguishing the different ranges of maryland beds per county pop.
...
**Answer** an even better map. Baltimore and Alleganey counties are set with number of beds per population, most baltimore area counties are set except for Howard. Southern Maryland is not set.
...
#logarythmic scale so that it its easier to decipher
#R has really good tools to get map data.
```
