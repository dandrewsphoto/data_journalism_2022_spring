...
**Answer** Take the PPP loans, and create a new dataframe called maryland_ppp_top_counties. Group them by county name, count the number of loans in each county. Arrange them in descending order and only show the top 10. Then show the new dataframe that we created.
...
maryland_ppp_top_counties %>%
  ggplot()
#you draw in ggplot in a sense, you're adding more as you go along. Unlike a pipe, where you're taking away.
```
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** a bar graph with some wonky titles.

```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=project_county_name, weight=total_loans))
#geom_bar means itll be a bar graph
#x avis is county names, Y axis is weight
```
...
**Answer** a new bargraph, organized by ascending order.

```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans))
#reorder --> organizes by ascending order
```
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** It was flipped on its side making it more readable. 
...
maryland_ppp_top_counties %>%
  ...
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
  coord_flip()
#coord_flip() flips the access, makes the graph and words more readable
```
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? 
**Answer** An updated bar graph with the background gone, can see the lines more clearly.
...
maryland_ppp_top_counties %>%
  ...
  coord_flip() + 
  theme_minimal()
#theme_minimal() gets rid of the background, can see the lines and everything else more clearly
```
...
**Answer** Difference between this plot and this one. Code installed to run gg themes.

```{r}
install.packages('ggthemes')
library(ggthemes)
#its own package that gives you a color pallet to work with. Basically the CSS for R.
```
...
**Answer** The bar graph is now styled wih the blue and black theme from the economist theme. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
  coord_flip() + 
  theme_economist()
#theme_economist() comes from gg theme library
```
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** labels are updated, it looks much better.

```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  #notice that there are + marks that add to the ggplot instead of the %>% which narrow down the code, line by line.
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
  coord_flip() + 
  theme_economist() +
  labs(
    ...
    x = "County\n",
    y = "\nTotal Loans",
    #axis were originally flipped, but because we did the coord flip earlier, it put them on the additional axis.
    #ALSO /n is a way to create some padding. Its essentially adding some space by literally inserting an extra line.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** A table of loans approved in chronological order with the number of loans approved on each date.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** A line graph that shows the difference from day to day of how many loans were approved each day over a certain period of time.

```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans))
#geom_line instead of geo_bar
#must have y= for line graphs, and weight= for bar graphs.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** An improved line graph with specified timeframes on the x-axis.
...
#date labels - lowercase b is abbreviating the month, uppercase Y because its giving you the full year number.
#If you wanted the full month, you'd use the uppercase B. Just like if you wanted a '96 year abbreviation, you'd use a lowercase y.
#can google data format strings in R
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** adjusted labels for readability.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
