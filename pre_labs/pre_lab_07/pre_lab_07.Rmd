...
**Answer** Tables and data that are embedded into the page, not able to be downloaded.
...
#save the url with the table data as an object. In the enviornment window in R, it is stored as a "Values" object.
```
...
#if Right Click doesn't come up, put in view-source: before the https of the website.

### Task 3: View source
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** source code for the entire page with tables and tags.
...
**Answer** It's showing that its an html doc written in English, and then divided into two parts: the head and the body. 

```{r}
# read in the html
naics_industry <- naics_url %>%
  read_html()
#starting with the object we created from the source URL and using the read_html() function to ingest all of the page html. We're then storing that as an object called naics_industry. As mentioned below, naics_industry is not a dataframe. It is a "nested list" that shows the general page structure of the html. 
...
**Answer** starting with the object we created from the source URL and using the read_html() function to ingest all of the page html. We're then storing that as an object called naics_industry. naics_industry is not a dataframe. It is a "nested list" that shows the general page structure of the html. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** The Inspect window, which shows the HTML and CSS of the part you right-clicked on to inspect. 
...
**Answer** It's all of the tables on the web page broken up into a bunch of different tibbles. In the enviornment window, naics_industry now has the tibbles all grouped. 
...
naics_industry <- naics_url %>%
  read_html() %>%
  html_table()
#start with the naics_url, and with that, read the html, and then read the table. save it to update as naics_industry.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** We started with the naics_url, and with that, read the html, and then read the tables. We saved it to update as naics_industry. They now show all of the tables on the web page within naics_industry.
...
**Answer** naics_industry[[1]] shows the first table that appears on the website, named "
Table 1. NAICS Conversion Implementation Dates for the CES Program" on the site. 4 rows, 3 columns. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** naics_industry[[2]] shows us the 2nd table listed on the sight, named "Table 2. NAICS Sectors". A key of sector number ids and their corresponding sector names, along with some junk at the bottom that we'll be cleaning out.
...
**Answer** A 21x2 tibble with sector code and sector name, along with junk to clear out in row 21.
...
naics_industry  <- naics_url %>%
  read_html() %>%
  html_table()
# for some reason we DONT "continue" the code with "%>%" before the next part.
#update naics_industry, which we're pulling from naice_url to create, read the html, read the tables....

# Just keep the second dataframe in our list

naics_industry <- naics_industry[[2]]
#...and then just take the second dataframe that appears in naics_industry and save THAT as naics_industry, discarding the 5 other tables we scraped from the website.
...
**Answer** a 20x2 tibble with the junk cleared out from row 21.
...
**Answer** We stored the url as an object called naics_url. Then, we pulled from that to create an object called naics_industry. To do that, we read in the html & read in the tables. We only wanted to keep the second dataframe, so we noted that and updated the naics_industry object to only have the 2nd tibble we pulled from the site. We cleaned the column names and sliced off row #21. We showed the updated naics_industry, which is a clean dataframe of 20 rows with naics sector ids and their corresponding names.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Employment and Unemployment data for sector 22, the Utilities sector. A bunch of tables that show gross gain and loss overtime, wages, retirement, health care, etc stats for sector 22. 
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer** an overview of the structure of the html page.
...
**Answer** The output is an overview of the structure of the html within the selected table.
...
#from the url, read the html and then use html_element with the accompanying xpath syntax to target the specific table.
...
**Answer** combining html_element and html_table gives us a proper looking tibble.
...
employment_info <- url %>%
  ...
  html_element(xpath = '//*[@id="iag22emp1"]') %>%
  html_table()
#combining html_element and html_table gives us a proper looking tibble. 
...
**Answer** a 1x2 tibble with just the value for employment listed.
...
employment_info <- url %>%
  ...
  html_table()
#again, no %>% after html_table!!!!

# Keep only second row with seasonally adjusted, bind back to each_row_df
employment_info <- employment_info %>%
  clean_names() %>%
  slice(2) %>%
  #instead of -2 to get rid of row #2 in the previous example, here we are using a positive integer to only keep that row.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Workforce Statistics, Employment and Unemployment, wages, etc data for sector 21, the Mining, Quarrying, and Oil and Gas Extraction sector.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Workforce Statistics, Employment and Unemployment, wages, etc data for sector 23, the Construction sector.
...
**Answer** It shows the very list of items listed to print, all with the number 1 next to them.
```{r}
...
**Answer** a list of all of the sectors.
```{r}
...
#The c() function tells R that we're making a list!
#note that the syntax is ("Firstword, Secondword and Thirdword", ..) with quotes around the specific names and with commas outside of each list item.
```
...
**Answer** We took the list_of_sectors we created above, and then ran the print function on it by using the for loop function. This gave us the same list of sectors, with the number 1 value next to it.
```{r}
...
for (sector in list_of_sectors) {
  print(sector)
#"for each sector in the object we called list_of_sectors, print the sector."
}

#took the list_of_sectors we created above, and then ran the print function on it by using the for loop function.

#for - saying we're running a loop
#(sector - the thing we're trying to get
#in - where we're looking to get the info
#list_of_sectors) { - the dataset/object we're pulling from
#print(sector) } - run the command print on the sector column that appears in list_of_sectors
...
**Answer** Error in sector_list : object 'sector_list' not found. It can't find the object "sector_list" its supposed to be looking in.
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** a working list of sectors
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** Error in print(sector_name) : object 'sector_name' not found. it cant find anything in sector_name we didn't call something sector_name yet.
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer** It prints out each number from 1 to 10.
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
  #"for each number in a range of numbers from 1 to 10, print the number."
}
```
...
**Answer** A tibble of 20 rows that shows the sector id code with its corresponding sector name.
...
**Answer** nrow produces a value, which is 20. 20 rows are within naics_industry.

```{r}
nrow(naics_industry)
#"display the number of rows within naics_industry."
```
...
**Answer** a print out of each row number from 1-20, as dictated by the number of rows in naics_industry.
...
#"for the row_number in the range of 1 to the number of rows found in naics_industry (which is 20), print each row number."
```
...
**Answer** Starting with the url, we read in the html, used html_element/xpath to target a specific table, and used html_table to clean it up into a tibble. We cleaned the column names, kept only the second row of that table, and selected to display only the data_series and nov_2021 columns. Since we saved it to employment_info, employment_info is a table that only shows the specific piece of data that we're looking for: a dataset of 2 columns and one row: the data name and the value.
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer** A cleaned, 20x2 tibble that has the naics sector codes and their accompanying names.
```{r}
# Define url of page we want to scrape
...
**Answer** a 20x3 tibble with the corresponding url now linked to the sector code and name.
...
#this is saying: create a new column called sector_url, propogating it using the command paste0 with "the beginning of the url up to the point that is the same for every url",sector, (which is a column name that will give us the value for each corresponding sector code in that row) ".htm")

#notice the syntax of apostrophes appearing just around the exact matches, not around sector. Also note the fact that the commas directly surround the sector item.
...
**Answer** A 19x4 tibble with the sector number, sector name, the corresponding url to each sector, and the corresponding xpath_id (table) for each sector, filtering out Public Administration.
...
#this ! filters OUT specific line items.
...
**Answer** 19 individual tibbles with each industry's code, name, url and xpath target. --> all contained with the list object each_row_df.
...
for(row_number in 1:nrow(naics_industry)) {
    ...
    each_row_df <- naics_industry %>%
      slice(row_number)
    #saving each row as its own little dataframe
...
#"Make a list with the row numbers from 1 to the number of rows in our naics_industry dataframe (which is 19). Then, for each element of that list (1, 2, 3, 4, 5 and so on up to 19), use slice() to keep only the one row that matches that number and save this newly created dataframe as each_row_df. Print out the dataframe. Then go to the next element on the list and do the same thing.  Keep doing that until we hit number 19, then stop." 


#OR BROKEN DOWN BY PIECE: 

#for(row_number in 1:nrow(naics_industry)) {
#"Make a list with the row numbers from 1 to the number of rows in our naics_industry dataframe (which is 19)."
# "Then, for each element of that list (1, 2, 3, 4, 5 and so on up to 19)..."

    #each_row_df <- naics_industry %>%
    #slice(row_number)
    #"...use slice() to keep only the one row that matches that number and save this newly created dataframe as each_row_df."

     
    #print(each_row_df)
    #"Print out the dataframe."
    #("Then go to the next element on the list and do the same thing. Keep doing that until we hit number 19, then stop.")
...
**Answer** A definition of 19 row numbers, their urls and their xpath ids.
```{r}
...
**Answer** All of the employee info. Not exactly sure what else I'm looking at here.
...
**Answer** 19 1x6 tibbles, showing Employment, all employees  (seasonally adjusted) for each industry.
...
**Answer** 19 dataframes with Employment, all employees (seasonally adjusted) information now with the accompanying industry sector code.
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer** A cleaned up 1x3 tibble with cleaned up column names, then selected the only three columns we need.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Labor stats on the Mining, Quarrying, and Oil and Gas Extraction sector: NAICS 21.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Labor stats on the Agriculture, Forestry, Fishing and Hunting sector: NAICS 11.
...
**Answer** The value for the number of employees for Agriculture is now NA.
