...
**Answer** Rows: 11397 Columns: 13, Each row is a different day, with data for how many shots were administered for which dose per county each day.
...
#read_csv doesn't jsut read local files. It can also read URLs if you're connected to the internet.
#Vaccination data - "county" data might represent people who got the shot in the county, but not necessarily people who live in that county.
```
...
#replace vaccination date with just the date, not date and time because the time is all the same and therefore not meaningful.
#once R knows it has date information in it, you can change it to any format you want.
```
...
**Answer** There are 2 more county entires than counties in Maryland.
...
**Answer** It is looking at the data of vaccines delivered where the county is listed as NA.
...
county_vaccinations_by_date
#exclamation point is the OPPOSITE of the function that follows it. 
```
...
#Loading files from the Internet, that's what's going to be changing. We want to be able to compare today's file with yesterday's file, no matter what the date is.

#The latest date is March 6th, because that's when Maryland stopped updating this data or publishing it here.
```
...
**Answer** Most recent 7-10 days are all ranked low among highest vaccination rates. 
...
#and its saying a week ago it was 122 people vaccinated, now its 32 . so its down 70%. 
```
...
**Answer** There was a rush to get vaccinated in April when they became widely available, then it started to severely drop off even as early as June. It then settled in with a few variations, especially in December when Omicron started to spike.
...
#if maryland kept updating its data, you could run this script every day and get the automated change in percentage. 
sub
```
...
**Answer** The number of cases in each county over time with a consistent, severe spike around January 1st 2022.
...
**Task** Run the following code and describe what the resulting chart shows
**Answer** another way of showing cases overtime but in an exponential value?
...
**Answer** number of cases per county as of April 18, 2022.
