...
**Answer** a 5 variable dataframe with the state, who won it in 2016, the amount of the vote that went to Trump, the percent that received ppp loans, and the loan amount received per small business employee.
```{r}
...
**Answer** x axis = percent for trump, y axis = businesses that got loans. the result is somewhat of a correlation, but not a causation. The variations between the plot points suggest it was kind of a mixed bag.  
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** a weaker relationship between the correlation. The line does move from lower left to upper right, but slightly. And there's an even bigger difference between the scatter plotted points.
```{r}
...
**Answer** a ranking of correlation. Trumpier states that received more ppe loans have an r score of .5, while Trumpier states that have more small business employees per state have an r score of .2, supporting our findings above. 
```{r}
reveal_data %>%
...
**Answer** 95 percent confidence interval, .52 cor rating. p value is 0.00008607, or very low.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** p-value = 0.1189, which is higher than the last one, meaning a higher chance of an aberration. cor value of 0.22. 
```{r}
...
**Answer** Trump states got a higher average ppp loan amount per the number of small business employees, as well as a higher amount of loans to small businesses than in Clinton states.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** t = -1.2223, p-value = 0.2295, meaning our findings are likely accurate but may not be alarming.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** t = -3.182, p-value = 0.002643. Since t is above 3, its particularly meaningful and worthy of further examination.
```{r}
