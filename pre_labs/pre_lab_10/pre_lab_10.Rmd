...
**Answer** Copy the url of the graphic you published here. --> https://www.datawrapper.de/_/vpdx5/
...
**Answer** Describe what the code below is doing and the output. The Code is taking the MD PPP data, creating a new dataset called md_counties, grouping by county and counting the number of loans in each county.
...
**Answer** Describe what the code below is doing and the output. The code is taking the summary of counties and only showing instances that have more than 6 loans in each county.
...
library(tigris)
#This installs FIPS codes. FIPS codes are a combo of the State Code (24 for Maryland) and a 3 digit code assigned alphabetically for the corresponding state's county names.
```
...
#I'm uessing fips_codes is an available dataset once you install tigris package and library...
#and then you can save it to a tible and name it how you want.
...
This code combines the state code column with the county code column into a new column that has the FIPS code for each county in Maryland.
...
md_fips <- md_fips %>% mutate(match_county = str_to_upper(county)) %>%
  #made the county column uppercase
...
#we can now join the MD fips and MD County data together. 
```

**Task** Now we'll join `md_counties` and `md_fips` together using our new `match_county` column:
**Answer** Describe here what you see in `md_counties_with_fips`. The counties are uppercase and cleaned up, joined with the number of loans and the FIPS code.
...
**Answer** Describe the changes from the previous map. --> Instead of a map of the US, its now a colorful map of Maryland.
...
**Answer** Describe the changes from the previous map. -> The counties appear and are cleaned up when you hover over them. The cleaned and formatted number of loans also appear with the County name when you hover over them.
...
**Answer** Put the URL of your map here. -> https://www.datawrapper.de/_/o8KWV/
